...
def plot_2d_embedding(embeddings, targets, ax, xlim=None, ylim=None, n_classes=2):
    
    sns.kdeplot(embeddings[:,0], embeddings[:,1], ax=ax, cbar=False)
    
    for c in range(n_classes):
        inds = np.where(targets==c)[0]
        ax.scatter(embeddings[inds,0], embeddings[inds,1], alpha=0.8, label="Class {}".format(c))
    if xlim:
